...
const BorrowedBooks = () => {
  ...
  const handleMarkAsReturned = async (borrowing) => {
    if (borrowing.returned_at) return;
    try {
      await API.put(`/borrowings/${borrowing.borrowing_id}/return`);
      setBorrowings(borrowings.map(b => b.borrowing_id === borrowing.borrowing_id ? { ...b, returned_at: new Date().toISOString() } : b));
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const columns = [
    ...
    { key: 'borrowed_at', label: 'Borrowed At', render: v => new Date(v).toLocaleDateString() },
    { key: 'due_date', label: 'Due Date', render: v => new Date(v).toLocaleDateString() },
    { key: 'returned_at', label: 'Returned At', render: v => v ? new Date(v).toLocaleDateString() : '-' },
    { key: 'fine_amount', label: 'Fine Amount' },
    { key: 'status', label: 'Status', render: v => <span className={v==='Returned'?'text-green-600 font-semibold':'text-blue-600 font-semibold'}>{v}</span> }
  ];

  const actions = [
    { name: 'Mark as Returned', onClick: handleMarkAsReturned, disabled: r => r.status === 'Returned', className: 'bg-green-500 text-white hover:bg-green-600' }
  ];
